...
def detect_double_dribble(ball_position, ball_previous_position, frame, ball_stationary_frames, ball_stationary_threshold=5):
    """Detects a possible double dribble violation."""
    
    if ball_position and ball_previous_position:
        # Calculate the movement of the ball
        ball_movement = np.linalg.norm(np.array(ball_position) - np.array(ball_previous_position))
        
        # If ball hasn't moved for a certain number of frames, it's stationary
        if ball_movement == 0:
            ball_stationary_frames += 1
        else:
            ball_stationary_frames = 0  # Reset if ball is moving again

        # If ball has been stationary for more than the threshold, suspect double dribble
        if ball_stationary_frames >= ball_stationary_threshold:
            cv2.putText(frame, "Double Dribble Violation", (50, 200), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
            return True, ball_stationary_frames
    return False, ball_stationary_frames
...
def process_violations(frame, tracked_objects, ball_position, ball_previous_position, player_positions, frame_width, frame_height, ball_stationary_frames):
    detect_out_of_bounds(player_positions, frame, frame_width, frame_height)
    double_dribble_detected, ball_stationary_frames = detect_double_dribble(ball_position, ball_previous_position, frame, ball_stationary_frames)
    detect_foul(tracked_objects, frame)
    return ball_stationary_frames
...
while True:
    ...
    for box in detections:
        ...
        if conf > 0.5:  # Filtering based on confidence threshold
            ...
            player_id = None
            if label == 'person':
                player_id = 1  # Assuming only one player
            if player_id:
    ...
    ball_stationary_frames = process_violations(frame, tracked_objects, ball_position, ball_previous_position, player_positions, frame_width, frame_height, ball_stationary_frames)
    
    for x1, y1, x2, y2, conf in detections_array:
        draw_bounding_box_and_text(frame, x1, y1, x2, y2, label, conf, player_id=player_id)

    # Update FPS in queue for real-time plotting
